...
import { computePlanResume } from "../src/modules/plans/plans.service";
...
describe("computePlanResume", () => {
    ...
    it("restores to 'due' when the plan was already overdue at the moment it was paused", () => {
        ...
        expect(result.restoredStatus).toBe("due");
        expect(result.daysPaused).toBe(2);
        expect(result.newNextDueAt).toBe("2026-08-12");
    });
    ...
    it("is zero when resumed the same day it was paused", () => {
        ...
        expect(result.newNextDueAt).toBe("2026-08-17");
        expect(result.restoredStatus).toBe("active");
    });
    ...
    it("composes across a second, later pause/resume cycle on the same booking", () => {
        ...
        expect(first.newNextDueAt).toBe("2026-08-20");
