...
--
-- Part 2 of 2 (see 20260813100050 for the enum addition this depends on —
-- renamed from 20260813100000 during the DPDPA merge because that version
-- collided with the relocate-pickup-hub migration).
--
-- MADE IDEMPOTENT DURING THE MERGE. staff_permissions already exists in the
-- hosted project (applied by hand, because the version collision above meant
-- the CLI skipped part 1 and this file was never recorded either). A bare
-- `create table` would therefore fail on the next push while still being
-- needed for a fresh `supabase db reset`. Guards make it work in both.
--
